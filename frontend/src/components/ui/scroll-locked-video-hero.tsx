  ...
  function getCtx(): AudioContext | null {
    try {
      if (!audioCtxRef.current) {
        // Safari still only exposes the prefixed constructor; typed
        // through `unknown` rather than `any` so the project's
        // no-explicit-any lint rule (which fails `next build`) passes.
        const Ctx =
          window.AudioContext ||
